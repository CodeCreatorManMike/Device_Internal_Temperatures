<#
.SYNOPSIS
General Microsoft / Surface laptop thermal report (convention-based zone mapping).

.DESCRIPTION
Use for any Microsoft Surface or Windows laptop when you do NOT have a model-specific script.
For Surface Pro 7 use Get-SurfaceTemperatures-SurfacePro7.ps1; for Surface Book 2 use Get-SurfaceTemperatures-SurfaceBook2.ps1; for Pro 8+ use Get-SurfaceTemperatures-SurfacePro8Plus.ps1.

.NOTES
Run: powershell -ExecutionPolicy Bypass -File ".\Get-SurfaceTemperatures-General.ps1"
#>
...
function Query-LegacyProbe { param($DiagRoot)
    ...
    try {
        ...
        foreach ($p in $probes) {
            if ($null -ne $p.CurrentReading) { $DiagRoot.LegacyProbe.AnyCurrentReading = $true; break }
        }
    } catch { }
}

function Query-SurfaceDrivers {
    param([System.Collections.Generic.List[string]]$DriverList, $DiagRoot)
    try {
...
function Write-HumanReadableReport {
    ...
    Write-Host " General Microsoft / Surface Thermal Report" -ForegroundColor Cyan
    ...
    if ($ZoneList.Count -eq 0) { Write-Host "  (no zones reported)" } else {
        foreach ($z in $ZoneList) {
            $idx = $z.IndexOf('=')
            if ($idx -gt 0) {
                $namePart = $z.Substring(0, $idx)
                $tempPart = $z.Substring($idx + 1)
                $shortName = $namePart; if ($namePart -match '\\([^\\]+)$') { $shortName = $Matches[1] }
                Write-Host ("  {0}: {1} {2}" -f $shortName, $tempPart, $degC)
            } else { Write-Host "  $z $degC" }
        }
    }
    Write-Host ""
    Write-Host "--- Component mapping (convention-based) ---" -ForegroundColor Yellow
    $labels = @(
        @{ Key = 'CPU'; Label = 'CPU' }, @{ Key = 'GPU'; Label = 'GPU' }, @{ Key = 'Exhaust'; Label = 'Exhaust' },
        @{ Key = 'LocalHotspot'; Label = 'Local hotspot' }, @{ Key = 'Battery'; Label = 'Battery' },
        @{ Key = 'ChargingCircuit'; Label = 'Charging circuit' }, @{ Key = 'Skin1'; Label = 'Skin 1' },
        @{ Key = 'Skin2'; Label = 'Skin 2' }, @{ Key = 'Misc'; Label = 'Misc' }, @{ Key = 'PCH'; Label = 'PCH' }
    )
    foreach ($item in $labels) {
        $val = $TempsTable[$item.Key]
        if ($null -ne $val -and $val -ne $NOT_AVAILABLE) { Write-Host ("  {0}: {1:N1} {2}" -f $item.Label, [double]$val, $degC) } else { Write-Host ("  {0}: N/A" -f $item.Label) }
    }
    Write-Host ""
    Write-Host "--- All ACPI thermal zones (InstanceName = Celsius) ---" -ForegroundColor Yellow
    if ($ZoneList.Count -eq 0) { Write-Host "  (none)" } else { foreach ($z in $ZoneList) { Write-Host ("  {0} {1}" -f $z, $degC) } }
    Write-Host ""
    Write-Host "--- Surface thermal/system drivers ---" -ForegroundColor Yellow
    if ($DriverList.Count -eq 0) { Write-Host "  (none found)" } else { foreach ($d in $DriverList) { Write-Host "  $d" } }
    Write-Host ""
...
try {
    ...
    if ($tz.Count -gt 0) { $source = 'ACPI_THERMAL_ZONE'; $countDetected = $tz.Count }
